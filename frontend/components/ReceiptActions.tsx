...
export function ReceiptActions({ orderId, title, backHref }: ReceiptActionsProps) {
  return (
    <div className="print:hidden">
      <div className="mx-auto mb-8 flex max-w-5xl flex-wrap justify-between gap-4 px-6 pt-10">
        <div>
          ...
          <h1 className="mt-2 font-serif text-4xl text-ink">{title ?? `Order #${orderId}`}</h1>
        </div>
        <div className="flex flex-wrap gap-3">
          <Link
            href={backHref ?? `/orders/${orderId}`}
